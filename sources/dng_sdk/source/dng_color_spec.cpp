...
dng_color_spec::dng_color_spec (const dng_camera_profile &profile)
								
	:	fChannels (profile.ColorMatrix1().Cols())
	...
	
{
	if (fChannels > 1)
    {
        ...
        fAnalogBalance.SetIdentity (fChannels);
...
dng_vector dng_color_spec::XYToNeutral (const dng_xy_coord &xy)
{
    dng_matrix colorMatrix = FindXYZtoCamera (xy);

    return colorMatrix * XYtoXYZ(xy);
}
