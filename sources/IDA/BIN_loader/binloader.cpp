/*
    ...
    Copyright 2013, 2020 Alexey Danilchenko
...
void process_symbols(linput_t *li, DcsSegment& symbolSegment)
{
    ...
    while (tok)
    {
        ...
        if (qsscanf(tok, "%x", &address) == 1)
        {
			...
			if (*tok=='0')
            {
				// we have a function symbol - add entry
                add_entry(address, address, qstr.c_str(), true, SN_IDBENC);
...
// processes help.txt segment/resource - that has enumerations for some error codes and events
void process_help_enums(linput_t *li, DcsSegment& helpSegment)
{
    ...
    if (tok)
    {
        ...
        enum_t enumId = add_enum(-1, enumName, hex_flag());
...
const char* get_segment_class(const char* name){
    if (name &&
        (!qstrcmp(name, "_boot") ||
...
void add_segment(linput_t *li, DcsSegment& segm, uint32 romAddress)
{
    ...
    if (!validate_name(&qstr, VNT_IDENT) || !force_name(segmRomAddress, qstr.c_str()))
        msg("Cannot set segmion ROM name \"%s\" for the address 0x%08X\n", name, segmRomAddress);

    // try also to set a comment - full name
    set_cmt (segmRomAddress, name, false);

    if (!qstrcmp(segm.name, "_preload"))
    {
    ...
    if (streq(segm.name, "help.txt") && realSize>0)
        helpSegment = &segm;
    ...
    const char* segmClass = get_segment_class(segm.name);
                    ...
                    uint32 offset = 0)
{
    ...
    if (doName)
    {
        ...
        if (!validate_name(&qstr, VNT_IDENT) || !force_name(address, qstr.c_str()))
            msg("Cannot set symbol name \"%s\" for the address 0x%08X\n", name, address);
    }
...
void add_immr_segment()
{
    ...
    force_name(immrBase, "IMMR");
    force_name(immrPramBase, "IMMR_PRAM");
    ...
    if (immrFull && (doImmrSingleStruct || immr && immrPram))
    {
        ...
        if (!doImmrSingleStruct)
            create_data(immrBase, stru_flag(), prevEntry->offset+prevEntry->size, stImmrT);
...
int idaapi accept_file(qstring* fileformatname, qstring* processor, linput_t* li, const char* /*filename*/)
{
    ...
    if (fileformatname)
	    *fileformatname = "Kodak DCS firmware binary";
