...
{
    {0,    0, 0, 0, {0xA,  0xA}, "f1.0"},
    {3,    0, 0, 0, {0xC,  0xA}, "f1.2"},
    {8,    0, 0, 0, {0xE,  0xA}, "f1.4"},
    {0xB,  0, 0, 0, {0x11, 0xA}, "f1.7"},
    {0x10, 0, 0, 0, {0x14, 0xA}, "f2.0"},
    {0x13, 0, 0, 0, {0x18, 0xA}, "f2.4"},
    {0x18, 0, 0, 0, {0x1C, 0xA}, "f2.8"},
    {0x1A, 0, 0, 0, {0x20, 0xA}, "f3.2"},
    {0x1B, 0, 0, 0, {0x22, 0xA}, "f3.4"},
    {0x26, 0, 0, 0, {0x23, 0xA}, "f3.5"},
    {0x20, 0, 0, 0, {0x28, 0xA}, "f4.0"},
    {0x22, 0, 0, 0, {0x2D, 0xA}, "f4.5"},
    {0x23, 0, 0, 0, {0x30, 0xA}, "f4.8"},
    {0x2E, 0, 0, 0, {0x32, 0xA}, "f5.0"},
    {0x28, 0, 0, 0, {0x38, 0xA}, "f5.6"},
...
void cameraUpdateSettingsMamiyaDigital()
{
    cameraAperture      = cameraFindSetting(cameraSettingConvert(getAV, apertureConvert),
                                            listApertureMamiya);
    cameraShutter       = cameraFindSetting(getTV, listShutterMamiya);
...
{
    if (digitalBody && camShutterOpen && exposureTime && shutterOpenTime &&
    ...
    if (timeUsec()-lastMessageTime > 800000 && camSoft &&
        !camShutterOpen && !camShutterOpenDigital)
    {
        camSoft = false;
    ...
    if (camShutterOpenDigital && !camShutterOpen)
    {
        if (resCode & 0xFFFF0000 == 0xD0000 &&
            ...
            timeUsec() - shutterCloseTime <= 20000000)
            return 0;
        else
            return 0x12;
    }
...
void cameraMessageMamiya(byte* msgReceived, byte* msgSent)
{
    ...
    if (msgReceived[0] == 1)
    {
        ...
        else if (msgReceived[2] == 0x82)
        {
            ...
            if (!camSoft && (camReply != 1 || (getST & 4) == 0))
            {
                cameraSoftPress();
                camReplyDelay = true;
            }
...
void cameraTaskMamiya(void)
{
    ...
    while (1)
    {
        rTimeDelay = 200000; // 0.2sec - 200ms
        ...
        do
        {
            ...
            do
            {
                ...
                while (REQ is low)
                {
                    ...
                    if (timeUsec()-lastCameraReqTime <= rTimeDelay)
                        continue;

                    // if delay for a reply to last message has not passed - keep waiting
                    if (camReplyDelay && timeUsec()-lastMessageTime <= rTimeDelay)
                        continue;
            ...
            // ger the first byte
            bytesReceived[0] = cameraSpiGetPut(0, rTimeDelay);
            ...
            if (bytesReceived[0] == 0x7E)
            {
                ...
                bytesReceived[1] = cameraSpiGetPut(1, rTimeDelay);
                bytesReceived[2] = cameraSpiGetPut(0, rTimeDelay);
            }
            ...
            while (curByte < totalBytes)
            {
                ...
                messageIn[curByte] = cameraSpiGetPut(messageOut[curByte], rTimeDelay);
