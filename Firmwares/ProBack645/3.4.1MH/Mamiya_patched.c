...
bool   mamiyaSerialV1         = false;  // 0000222A - Is Mamiya Serial Protocol V1
byte   setP1                  = 0;      // 0000222B
bool   camSettingsReceived    = false;  // 000021DF
byte   curCamSoftOnTime       = 0;      // 000021B2 - current idx of camSoft on time
                                        //            allows to terminate CCD active 
                                        //            early after shot taken  

// new properties
uint32 manualLensSetting = 0;   // 000124BC
uint32 camSoftOnTime     = 0;   // 000124C0

// This is to distinguish AFD II/III from DF
bool   isFilmBody             = false;  // 000021F3 

// Camera soft press CCD activation times
char* camSoftOnTimesStr[] =     // 00012588
{
...
uint32 camSoftOnTimes[] =       // 00012558
{
...
TLensSettingPair manualLensData[] =     // 00014630
{
    {24,  11},    // 24mm  f/4   
    {35,  10},    // 35mm  f/3.5 
    {45,  7 },    // 45mm  f/2.8 
...
TProperty mamiyaProperties[2] =     // 00014460
{
    { "manualLensSetting", "Manual Lens Data",           &manualLensSetting, 
      "Mamiya/Phase One", LIST, 0x10502, 0, 20, 0, manualLensNames,0,0 },
...
// send/receive single byte via SPI
...
void cameraDisable()
{
    ...
    if (cameraContaxDialogTaskId)
    {
        cameraContaxDialogRequest++;
        return;
    }
...
uint32 imageTakePictureError(uint32 timeout)
{
    ...
    if (curTime + timeout - captureStartTime < imageCaptureTime)
        return 0xD0102;
    ...
    if (slowFlush)
    {
        if (timeout< 300000 &&
            (!ccdIsOn() || !imageFlushStarted ||
             curTime+timeout-flushStartTime<250000))
            return 0xD0004;
...
{
    if (digitalBody && camShutterOpen && shutterOpenTime)
    {   
        uint32 shutterOpenedPeriod = timeUsec()-shutterOpenTime;
        if ((exposureTime && shutterOpenedPeriod > exposureTime+100000) ||
            (exposureTime==0 && shutterOpenedPeriod > 20000 &&  // handle bulb exposure
             SPIMOSI is low))
        {
    ...
    if (camSoft && !camShutterOpen && !camShutterOpenDigital && 
        timeUsec()-lastMessageTime > camSoftOnTimes[camSoftOnTime])
    {
        // deactivate sensor
        camSoft = false;
    ...
    if (camShutterOpenDigital && !camShutterOpen)
    {
        if (resCode & 0xFFFF0000 == 0xD0000 &&
            ...
            if (timeUsec() - shutterCloseTime <= 10000000)
                return 0;
...
void cameraMessageMamiya(byte* msgReceived, byte* msgSent)
{
    ...
    if (msgReceived[0] == 1)
    {
        ...
        else if (msgReceived[2] == 0x82)
        {
            ...
            if (!camSoft && camEnabled && camReply == 1 && (getST & 4) == 0)
            {
                cameraSoftPress();
                camReplyDelay = true;
            }
...
{
    const REQ_TIME_DELAY   = 32000; // 32ms - 0x7D00
    const SPI_BYTE_TIMEOUT = 2000;  // 2ms  - 0x7D0
    ...
    propertyRegister(mamiyaProperties, 2);
    ...
    while (1)
    {
        do
        {
            ...
            do
            {
                ...
                // Wait for camera to lower REQ - at the end of the message
                while (REQ is high)
                    osEvPend(-1, 1, 1000, 0);

                // Wait for REQ to go high from camera (camera sends data to back)
                // or if back needs something - issue a request to camera
                while (REQ is low)
                {
                    ...
                    if (camAwake ||
                        timeUsec()-bodySleepTime <= 700000)  // 0.7s
                    {
                    ...
                    if (timeUsec()-lastCameraReqTime <= REQ_TIME_DELAY)
                        continue;

                    // if delay for a reply to last message has not passed - keep waiting
                    if (camReplyDelay && timeUsec()-lastMessageTime <= 200000)
                        continue;
            ...
            // reset buffers to 0
            memset(bytesReceived, 0, 44);
            memset(bytesSent,     0, 44);

            bytesReceived[43] = 0xAA; // mark end of received buffer

            cameraNeedsReply = false;
            startBytes = 0;

            // ger the first byte
            bytesReceived[0] = cameraSpiGetPut(0, SPI_BYTE_TIMEOUT);

            if (bytesReceived[0] == 0x7E)
            {
                startBytes = 2;
                if (cameraReqSent)
                {
                ...
                bytesReceived[1] = cameraSpiGetPut(1, SPI_BYTE_TIMEOUT);
                bytesReceived[2] = cameraSpiGetPut(0, SPI_BYTE_TIMEOUT);
            }

            cameraReqSent = false;

            byte* messageIn  = bytesReceived + startBytes;
            byte* messageOut = bytesSent + startBytes;
            byte* msgOutData = messageIn;  // by default echo incoming message back
            if (messageIn[0] == 1)
            {
                // REQ_output();
                cameraNeedsReply = true;
                SPIMISO_on();
            }

            byte totalBytes = messageIn[0] == 0x7D ? 2 : 8;
            byte checkSum = 0;
            byte curByte = 1;
            while (curByte < totalBytes)
            {
                if (messageIn[0] == 1)
                {
                    // message 1 needs response
                    messageOut[curByte] = *msgOutData;

                    if (curByte == 2)
                    {
                        if ((backMessageCount < 2 || digitalBody) && !noDigitalBody)
                            messageOut[curByte] = 0x81;
                        else
                            messageOut[curByte] = 0x11;
                    }
                    else if (curByte == 4)
                    {
                        totalBytes = messageIn[3] + 8;
                        if (messageIn[2] == 3)
                        {
                            camReplyDelay = false;
                            if (!digitalBody)
                            ...
                            else if (cameraReqCmd & 0x10)
                            {
                                ...
                                msg03d[4] = command[cameraReqCmd].cmd & 0xF0;
                                msgOutData = msg03d;
                                totalBytes = msg03d[1] + 8;
                            }
                            else
                            {
                                // use msg03 for AFD - digital body
                        ...
                        else
                        {
                            for (i=0; messageList[i]; i++)
                                ...
                                {
                                    msgOutData = messageList[i];
                                    totalBytes = messageList[i][1] + 8;
                                    break;
                                }
                        }
                    ...
                    else if (curByte + 2 == totalBytes)
                        messageOut[curByte] = checkSum;  // byte before last is checksum
                    else if (curByte + 2 > totalBytes)
                        messageOut[curByte] = (byte) 0;  // last byte 0

                    if (curByte >= 2)
                        checkSum ^= messageOut[curByte];

                    // advance the ouput msg data to next byte
                    ++msgOutData;
                }
                else
                    messageOut[curByte] = 0;

                messageIn[curByte] = cameraSpiGetPut(messageOut[curByte], SPI_BYTE_TIMEOUT);
                if (curByte >= totalBytes ||
                    messageIn[curByte] == 0xAA ||
                    REQ is low)
                    break;
                curByte++;
            }
            if (messageIn[curByte] == 0xAA && messageIn[curByte-1] != 0)
                mamiyaSerialV1 = true;

            SPIMISO_off();
            // REQ_input();

            cameraMessageMamiya(messageIn, messageOut);
        }
        while (!cameraWatch && !cameraLogWatch);

        cameraSpiLog(bytesReceived, bytesSent, startBytes + curByte + 1, cameraNeedsReply);
    }
}
