...
char* camShutterModeStr[] =     // 0001478C
{
    "Legacy (645 AF/AFD)",
...
TProperty mamiyaProperties[3] =     // 00014460
{
...
byte cameraMessageSetupMamiya(byte cameraReqCount)
{
    ...
    if (setST == 0)
    {
        if ((shutterMode == SM_DIGITAL_SHORT && camSoft &&
    ...
    setST |= shutterModeMask[shutterMode];
    ...
    if (msgReceived[0] == 1)
    {
        ...
        else if (msgReceived[2] == 0x83)
        {
            ...
            switch (msgType)
            {
                ...
                case 5:
                    if (shutterMode == SM_LEGACY && !camShutterArm)
                    {
                        camShutterArm         = true;
                        camShutterOpen        = false;
                        camShutterOpenDigital = false;
...
void cameraTaskMamiya(void)
{
    ...
    while (1)
    {
        do
        {
            ...
            uint32 spiTimeout = reqTimeDelay;
            ...
            // ger the first byte
            bytesReceived[0] = cameraSpiGetPut(0, spiTimeout);
            ...
            if (bytesReceived[0] == 0x7E)
            {
                ...
                bytesReceived[1] = cameraSpiGetPut(1, spiTimeout);
                bytesReceived[2] = cameraSpiGetPut(0, spiTimeout);
            }
            ...
            while (curByte < totalBytes)
            {
                ...
                messageIn[curByte] = cameraSpiGetPut(messageOut[curByte], spiTimeout);
